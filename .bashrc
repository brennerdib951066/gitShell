...
if [[  ${@} =~ get ]]; then
    ...
    status=$(grep -Ewoi "conexão|qualificação" <<< ${@})
...
t=$(printf "%s%22s\n%s\n%b%15s\n%s\n" "\`BOLTEN" "(${dataHoje})\`" "----------------------------" "*${qtdsStatus} ${status^}*" "|" "----------------------------")
pwsh -File "${arquivoNotificacao}" notificarWhatsApp "${t}" 'brenner'
#[[ ${status,} =~ "conexão" ]] && printf "%s%22s\n%s\n%b%19s\n%s\n" "BOLTEN" "(${dataHoje})" "----------------------------" "\e[33;2m${qtdsStatus} ${status^}\e[m" "|" "----------------------------" || printf "%s\n%s\n%b%14s\n%s\n" "BOLTEN:" "----------------------------" "\e[32;2m${qtdsStatus} ${status^}\e[m" "|" "----------------------------"

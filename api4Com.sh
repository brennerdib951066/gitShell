...
dataAtual="$(date -u -d '-4 days' +"%Y-%m-%dT00:00:00.000Z")"
dataAmanha="$(date -u -d '+1 days'  +"%Y-%m-%dT23:59:59.999Z")"
countPagina="2"
for ((i=0;i<${#ramal[@]};i++)); do
    ...
    for ((c=1;c<(($countPagina+1));c++)); do
        echo -e "\E[31;1mCCCCCCCCCCCCC = ${c}\E[m"
        export c
    ...
    soma=$(awk -v usuario="${usuario[i]}" '
        ...
        END { printf "%sTOTAL: %.2f", toupper(usuario), total }' <<< "$chamadas"
    )
